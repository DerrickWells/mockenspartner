...
class AESEncryptor {
	...
	static get SALT_SIZE() { return 256; }
	static get KEY_SIZE() { return 32; }
	static get IV_SIZE() { return 16; }

	get saltSize() { return this[$saltSize]; }
	set saltSize(byteCount) { this[$saltSize] = byteCount; }

	get keySize() { return this[$keySize]; }
	set keySize(byteCount) { this[$keySize] = byteCount; }

	get ivSize() { return this[$ivSize]; }
	set ivSize(byteCount) { this[$ivSize] = byteCount; }

	encrypt(plainText, password) {

		if (!plainText) { throw new TypeError('Invalid plaintext'); }

		if (!password) { throw new TypeError('Invalid password'); }
	...
	decrypt(cipherText, password) {
		...
		if (!cipherText) { throw new TypeError('Invalid ciphertext'); }
		...
		// load salt
		for (let i = 0; i < salt.length; i++) { salt[i] = packed[i]; }

		// load enciphered data
		for (let i = 0; i < enciphered.length; i++) { enciphered[i] = packed[i + salt.length]; }
